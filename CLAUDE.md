...
- **NEVER commit `.env`, model weights, or any real face image.** Evaluation corpora are **fetched, never vendored** — `infra/fetch_evalsets.sh` downloads into gitignored `fixtures/faces/` and records licences in `fixtures/faces/LICENSES.md`. Only the derived golden `.npy` is committed. Never attach a face image to a Linear issue, a PR, or a log.
- **NEVER use a withdrawn dataset or its mirrors** — MS-Celeb-1M, VGGFace2, MegaFace, CASIA-WebFace are all withdrawn, and copies persist. See `docs/PLAN.md` §9.1 for what is actually usable.
- **NEVER ship a threshold derived from a public benchmark.** It runs too low for a cooperative kiosk, so realised FAR is worse than predicted while FRR looks good — the dangerous direction for attendance. Public corpora give a starting point; the shipping value comes from in-domain captures. Freeze `(model hash, preprocessing path, threshold, metric)` as one release unit and re-derive on any model or camera change (§9.2).
...
- Codex project agent files under `.codex/agents/*.toml` are **role templates**, not automatically registered callable agents in the current Codex CLI. When parallel work is useful, use the available multi-agent tool and pass the relevant template instructions into the spawned agent prompt. If no multi-agent tool is available in the session, execute the work sequentially and say so.
