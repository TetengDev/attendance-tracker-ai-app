...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint("gps_accuracy_m IS NULL OR gps_accuracy_m >= 0", name=op.f("ck_scan_sessions_gps_accuracy_non_negative")),
        sa.CheckConstraint(
        ...
        sa.CheckConstraint("scan_count >= 0", name=op.f("ck_scan_sessions_scan_count_non_negative")),
        sa.CheckConstraint("start_lat IS NULL OR start_lat BETWEEN -90 AND 90", name=op.f("ck_scan_sessions_start_lat_range")),
        sa.CheckConstraint("start_lng IS NULL OR start_lng BETWEEN -180 AND 180", name=op.f("ck_scan_sessions_start_lng_range")),
        sa.ForeignKeyConstraint(["device_id"], ["devices.id"], name=op.f("fk_scan_sessions_device_id_devices"), ondelete="CASCADE"),
        sa.ForeignKeyConstraint(["location_id"], ["locations.id"], name=op.f("fk_scan_sessions_location_id_locations"), ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["operator_admin_id"], ["admin_users.id"], name=op.f("fk_scan_sessions_operator_admin_id_admin_users"), ondelete="SET NULL"),
        sa.PrimaryKeyConstraint("id", name=op.f("pk_scan_sessions")),
    )
    op.create_index("ix_scan_sessions_device_started_at", "scan_sessions", ["device_id", "started_at"], unique=False)
    op.create_index("ix_scan_sessions_location_started_at", "scan_sessions", ["location_id", "started_at"], unique=False)
    op.create_index(
...
def downgrade() -> None:
    op.drop_constraint(op.f("fk_attendance_events_session_id_scan_sessions"), "attendance_events", type_="foreignkey")
    op.drop_index("ix_attendance_events_session_id", table_name="attendance_events")
    op.drop_column("attendance_events", "session_id")
    op.drop_index("uq_scan_sessions_open_device", table_name="scan_sessions", postgresql_where=sa.text("ended_at IS NULL"))
