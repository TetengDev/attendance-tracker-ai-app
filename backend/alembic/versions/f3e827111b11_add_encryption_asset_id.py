...
def upgrade() -> None:
    ...
    op.add_column(
        "face_embeddings",
        sa.Column("encryption_asset_id", postgresql.UUID(as_uuid=True), nullable=True),
    )

    # Backfill using the existing asset_id (if it is null, we can't do much, but we assume it's mostly present for active ones)
    op.execute(
        "UPDATE face_embeddings SET encryption_asset_id = asset_id WHERE asset_id IS NOT NULL"
    )
    ...
    op.execute(
        "UPDATE face_embeddings SET encryption_asset_id = '00000000-0000-0000-0000-000000000000'::uuid WHERE encryption_asset_id IS NULL"
    )
