...
async def load_gallery_from_db():
    ...
    async with get_session_factory()() as session:
        result = await session.execute(select(FaceEmbedding).where(FaceEmbedding.is_active == True))
        ...
        for emb in embeddings:
            ...
            entries.append(
                GalleryEntry(person_id=emb.person_id, embedding_id=emb.id, vector=vector)
            )
